...
function getClusterInfo(config, context) {
  ...
  if (!clusterInfo) {
    throw new Error(
      `Unable to find cluster information for context ${context}`
    );
  }
  return clusterInfo;
}
...
function getKubernetesAPIURL(config) {
  ...
  clusterURL = _.startsWith(clusterURL, 'http')
    ? clusterURL
    : `http://${clusterURL}`;
  return clusterURL;
}
...
function getToken(userInfo) {
  const token = require('child_process')
    .execSync(
      'aws eks get-token --cluster-name eks-production --region eu-central-1|jq .status.token -r'
    )
    .toString()
    .trim();
  return token;
...
function warnUnsupportedOptions(
  ...
  unsupportedOptions.forEach(opt => {
    if (!_.isUndefined(definedOptions[opt])) {
      logFunction(
        `Warning: Option ${opt} is not supported for the kubeless plugin`
      );
    }
  });
}
...
function getRuntimeDepfile(runtime, configMap) {
  ...
  if (!depFile) {
    throw new Error(`The runtime ${runtime} is not supported yet`);
  }
  return depFile;
}
...
function getDeployableItemsNumber(functions) {
  return _.sum(
    [_.keys(functions).length].concat(_.map(functions, f => _.size(f.events)))
  );
}
